...
> {
  ...
  buildFilters(
    ...
    if (filters.start_date) {
      query.andWhere(
        "(commit.resource #>> '{authored_date}')::timestamptz AT time zone 'America/Vancouver' >= (:startDate)",
        {
    ...
    if (filters.end_date) {
      query.andWhere("(commit.resource #>> '{authored_date}')::timestamptz AT time zone 'America/Vancouver' <= (:endDate)", {
